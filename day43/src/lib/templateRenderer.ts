export const compileToString = async (template: string) => {
	...
	if (scriptContent) {
		scriptContent.map(async (scriptData) => {
			...
			if (scriptData.startsWith('<script>') && scriptData.endsWith('</script>')) {
				...
				// }
				const layoutElms = parseFromRegex(scriptData, /defineLayout\({ name: '(.*?)' }\)(;){0,1}/g);
				layoutElms.forEach((layoutElm: string) => {
					if (layoutElm.startsWith('defineLayout({')) {
						scriptData = scriptData.split(layoutElm).join('');
						template = template.split(layoutElm).join('');
						console.log(scriptData, layoutElm);
						...
						layout = JSON.parse(layoutObjString);
					}
				});
	...
	return { fnStr, styles, script: scriptInjection, head: headInjection, layouts: layout };
};
