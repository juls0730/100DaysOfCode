...
async function createServer() {
	...
	async function renderPage(url: string) {
		...
		if (scriptCache.get(fileName)) {
			const cachedObj = scriptCache.get(fileName);
			code = cachedObj;
			script = code.script;
		} else {
			const { renderSSRHydrationCode } = await vite.ssrLoadModule(basepath.slice(1) + 'lib/router/SSR/ssrHydrationGenerator');
			code = await renderSSRHydrationCode(appContent, metaObj.reduceJavascript);
			if (appHtml.script || code.script) {
				...
				let script: string | undefined = appHtml.setupScript +  code.script + ' window.addEventListener("load", async () => { ' + appHtml.script + serverSideScriptInjection + ' });';
				...
				scriptCache.set(fileName, { script, template: (code.template) });
			}
		}
