	...
	async function renderPage(url: string) {
			...
			await Promise.all(elements.map(async (componentInComponent: string | undefined) => {
				...
				if (componentInComponent === 'img') {
					...
					if (base64SVG && typeof base64SVG !== 'string') {
						component = component.replace(base64SVG.srcName, base64SVG.data);
					}
				}
		...
		await Promise.all(pageData.split('<').map(async e => {
			...
			if (item === 'img') {
				...
				if (base64SVG && typeof base64SVG !== 'string') {
					pageData = pageData.replace(base64SVG.srcName, base64SVG.data);
				}
			}
			...
			if (isSloted && slotData) {
				component = component.replace('<slot />', slotData);
			}
			let componentName = '<' + item;
			(!isSloted) ? componentName += ' />' : componentName = '<!--' + item + '-->';
			pageData = pageData.replace(componentName, component);
		}));
