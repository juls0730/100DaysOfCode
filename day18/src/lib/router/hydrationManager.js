...
import { isSSR } from '../../entry-client.js'
...
export async function hydratePage() {
    ...
    anchorElms.forEach((e) => {
        e.addEventListener('click', async (click) => {
            event.preventDefault()
            let { renderPage } = await import('./pageRenderer')
    ...
    if (isSSR()) {
        let linkPrefetcher = await import('./linkPrefetcher.js');
