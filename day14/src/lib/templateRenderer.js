export const compileToString = async (template) => {
    ...
    ast.map(async t => {
        ...
        // elements.forEach(async (e, index, array) => {
        //     if (!e) return
        //     let element = e.split(' ')[0]
        //     if (element.includes('/') || element.includes('{') || element.includes('}')) return
        //     if (!isHTML(element.split('>')[0])) {
        //         let component
        //         if (!import.meta.env.SSR) {
        //             await fetch('/src/components/' + element.split('>')[0] + '.devto').then((response) => {
        //                 if (response.ok) {
        ...
        //                 path.resolve('./src/components/' + element.split('>')[0] + '.devto'),
        //                 'utf-8',
        //             )
        //         }
        //         component = await compileToString(component)

        //         component = '`' + component.slice(2) + '+`'

        //         if (!component) return;
        //         const componentsElement = '<' + element + ' />'
        //         fnStr = fnStr.replace(componentsElement, component)
        //         console.log('compiled component')
        //     }
        // })

        console.log('running extra render steps')
