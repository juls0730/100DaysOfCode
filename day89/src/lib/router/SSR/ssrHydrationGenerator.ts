		...
		eventElements.forEach((e) => {
			...
			dOnAttrs.forEach((attr) => {
				...
				if (!isKeyboardEvent || !keyName) {
					script += `eval(unescape(\`${inlineCode}\`));`;
				} else {
					script += `if (eventKey === "${keyName}") {
						eval(unescape(\`${inlineCode}\`));
					}`;
				}

				script += `}, { once: ${modifiers.some(mod => mod === 'once')}, capture: ${modifiers.some(mod => mod === 'capture')}, passive: ${modifiers.some(mod => mod === 'passive')} });`;
			});
		});
	}
