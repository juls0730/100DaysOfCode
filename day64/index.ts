...
async function createServer() {
	...
	async function renderPage(url: string) {
		...
		if (appHtml.script || code.script) {
			...
			let script: string | undefined = appHtml.setupScript + ' window.addEventListener("load", async () => { ' + code.script + appHtml.script + serverSideScriptInjection + ' });';
			...
			injection = (await terser.minify(script, options)).code;
			//scriptCache.set(fileName, script);
		}
		// }

		if (injection) {
			$('html', null).children('head').append('<script async type="module">' + (injection) + '</script>\n');
		}
