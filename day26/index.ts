...
async function createServer() {
	...
	async function renderPage(url: string) {
		...
		if (cache[fileName] && process.env.NODE_ENV == 'production') {
			const html = JSON.parse(cache[fileName]);
			return { html, status };
		}
		...
		if (appHtml.script || code) {
			...
			scriptedTemplate = template.replace('<!--script-outlet-->', '<script async type="module">' + code + appHtml.script + '</script>');
		}
		...
		const headedTemplate = stylizedTemplate.replace('<!--head-outlet-->', appHtml.head);

		const html = headedTemplate.replace('<!--ssr-outlet-->', eval(appHtml.fnStr));
		return { html, status };
	}
	...
	app.use('*', async (req: Request, res: Response, next: NextFunction) => {
		...
		try {
			...
			cache[fileName] = JSON.stringify(page.html);
