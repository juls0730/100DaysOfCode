...
export async function hydratePage(reduceJavascript = false) {
	...
	const hydrateEvent = (eventName, elementSelector, removeAttribute = true) => {
		hydrateElement(elementSelector, appState, eventName, getCookie, setCookie, removeAttribute);
	};
