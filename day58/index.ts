...
async function createServer() {
	...
	async function renderPage(url: string) {
		...
		async function renderComponent(component: string) {
			...
			await Promise.all(elements.map(async (componentInComponent: string | undefined) => {
				...
				if (componentInComponent === 'img') {
					const base64SVG = renderSVGToBase64(ogComponent);
					if (base64SVG && typeof base64SVG !== 'string') {
						component = component.replaceAll(base64SVG.srcName, base64SVG.data);
					}
				}
		...
		await Promise.all(pageData.split('<').map(async e => {
			...
			if (!item) return;
			if (item === 'img') {
				const base64SVG = renderSVGToBase64(item);
				console.log(base64SVG);
				if (base64SVG && typeof base64SVG !== 'string') {
					pageData = pageData.replaceAll(base64SVG.srcName, base64SVG.data);
				}
			}
			...
			const slotedComponent = pageData.split('<' + item + '>');
			let isSloted = false;
			let slotData: string | undefined;
			if (slotedComponent.length > 1) {
				isSloted = true;
			...
			(!isSloted) ? componentName += ' />' : componentName = '<!--' + item + '-->';
		...
		if (appHtml.script || code) {
			...
			appHtml.setupScript = 'document.documentElement.classList.add("loading");' + appHtml.setupScript;
			...
			if (metaObj.suspendUntilHydrated) {
				...
				script += 'document.addEventListener("router:client:load", () => {document.documentElement.classList.remove("loading")});';
			}
			const options = {
				mangle: false,
				module: true
