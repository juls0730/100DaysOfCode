...
export async function hydratePage(reduceJavascript: boolean) {
	...
	hydrateSubmit(appState, documentBody);
}
...
export function hydrateSubmit(appState: Reactive, rootElement: Element) {
	Array.from(rootElement.querySelectorAll('*')).forEach((e: Element) => {
		const submitElms = Array.from(e.attributes).filter((arrElm: Attr) => {
			return arrElm.name.startsWith('d-on:submit');
		});

		if (submitElms.length === 0) return;
		submitElms.forEach((attr) => {
			const attributeName = attr.name;
			const submitExtension = attributeName.split('.')[1]?.toLowerCase();
			e.addEventListener('submit', (event: Event) => {
				if (submitExtension === 'prevent') {
					event.preventDefault();
				}

				eval(e.getAttribute(attributeName));
			});
		});
	});
}
