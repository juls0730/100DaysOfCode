...
export async function hydratePage(reduceJavascript: boolean) {
	...
	hydrateElements(appState, reduceJavascript);
}
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function hydrateElements(appState: Reactive, reduceJavascript: boolean) {
	if (reduceJavascript === undefined) reduceJavascript = false;
	...
	const anchorElms = elements.filter((e) => {
		return Array.from(e).filter(el => el.tagName === 'a');
	});
